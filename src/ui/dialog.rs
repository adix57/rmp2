...
impl Search {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> SearchOutcome {
        ...
        match key.code {
            ...
            KeyCode::Enter => SearchOutcome::Done(self.text.clone()),
            KeyCode::Backspace => {
                if self.cursor > 0 {
            ...
            _ => SearchOutcome::None,
        }
    }
}

pub fn search_box(frame: &mut Frame, area: Rect, search: &Search, count: usize, total: usize) {
    ...
    let rect = centered(area, 46, 5);
    frame.render_widget(Clear, rect);
    frame.render_widget(block, rect);

    let input = format!("> {}", search.text.clone());
    let line = if search.cursor < input.len() {
        let mut cur = input.clone();
        cur.insert(search.cursor + 2, '|');
        cur
    } else {
        format!("{input}|")
    };
    let text = vec![
        Line::from(line),
        Line::from(format!(
            "  {count}/{total} matches   (enter apply, esc clear)"
        )),
    ];
    frame.render_widget(
